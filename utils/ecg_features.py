...
def get_ecg_features(ecg, time_in_sec, fs):
    """Extract HR statistics, HRV, SNR, flatline ratio, and baseline wander from a raw ECG segment.

    Quality metrics (flatline, baseline wander) are always computed and returned
    even when R-peak detection fails — cardiac metrics fall back to NaN in that case.

    Returns
    -------
    np.ndarray of shape (7,):
        [hr_mean, hr_max, hr_min, hrv, snr, flatline_ratio, baseline_wander_ratio]
    """
    # --- Quality metrics: no R-peaks required ---
    ...
    # Baseline wander on the fully cleaned signal (after bandpass + ecg_clean)
    bw = baseline_wander_ratio(ecg_cleaned, fs)

    # --- R-peak dependent cardiac metrics ---
    try:
        _, rpeaks = nk.ecg_peaks(ecg_cleaned, sampling_rate=fs, method="engzeemod2012")
    except Exception:
        return np.array([np.nan, np.nan, np.nan, np.nan, np.nan, flat, bw])
    ...
    return np.array([hr_mean, hr_max, hr_min, hrv, snr, flat, bw])
